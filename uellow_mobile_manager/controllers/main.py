...
def _uc_remember_order(order):
    ...
    the success page afterwards. Keeps the last 20 ids. Mirrors the guard in
    uellow_checkout so both copies of this controller behave identically."""
    try:
...
def _uc_may_view_order(order):
    """True only if the caller may see this order's details on the public
    success page. Prevents an anonymous visitor from enumerating
