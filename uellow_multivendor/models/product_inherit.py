...
class ProductTemplate(models.Model):
    """Extend product.template with vendor, approval workflow, and flash sale."""
    ...
    vendor_rejection_reason = fields.Text(
        'Rejection Reason', copy=False,
    )
    vendor_submitted_by = fields.Many2one(
        'res.users', string='Submitted By',
        ondelete='set null', copy=False,
    )
    vendor_submitted_date = fields.Datetime(
        'Submitted On', copy=False,
    )
    vendor_reviewed_by = fields.Many2one(
        'res.users', string='Reviewed By',
        ondelete='set null', copy=False,
    )
    vendor_reviewed_date = fields.Datetime(
        'Reviewed On', copy=False,
    )
    vendor_admin_notes = fields.Text(
        'Admin Notes (internal)', copy=False,
    )

    # ── Flash Sale ───────────────────────────────────────────────
    flash_sale_id = fields.Many2one(
        'uellow.flash.sale', string='Active Flash Sale',
        ondelete='set null',
    )
    ...
    def get_display_price(self):
        """Return flash sale price if active, else list price."""
        self.ensure_one()
    ...
    def action_vendor_approve(self):
        """Admin approves product — publish on website."""
        for product in self:
            product.write({
            ...
            if product.vendor_id and product.vendor_id.user_id:
                product.message_post(
                    body=_('✅ Your product <b>%s</b> has been approved and is now live on the store.') % product.name,
                    partner_ids=[product.vendor_id.user_id.partner_id.id],
                    message_type='notification',
                )

    def action_vendor_reject(self):
        """Admin rejects product — opens wizard for reason."""
        return {
