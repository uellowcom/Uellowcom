...
(function () {
    ...
    function ppcRelocateCTAExtras() {
        // WhatsApp stays INLINE in the CTA row — see PHASE 14 note in
        // product_page.css. We used to float it as a FAB, but the user
        // asked to remove the float and keep the in-row button instead.

        // ── Fast Order → "Quick Buy" callout card above the CTA row ─
        var zoWrap = document.querySelector(".zo-wrap");
        var ctaRow = document.getElementById("o_wsale_cta_wrapper");
        if (zoWrap && ctaRow && !document.querySelector(".ppc-quickbuy-card.ppc-mounted")) {
            // We need to keep the original button alive (zorder.js binds
            // listeners to .zo-btn-open) — so we relocate the entire .zo-wrap
            // by transplanting its inner button into our new card and
            // forwarding clicks. Simpler: hide the original via CSS and
            // build a new clickable card that calls the same handler.
            var card = document.createElement("div");
            card.className = "ppc-quickbuy-card ppc-mounted";
            card.setAttribute("role", "button");
            card.setAttribute("tabindex", "0");
            card.innerHTML =
                '<div class="ppc-qb-icon" aria-hidden="true">⚡</div>' +
                '<div class="ppc-qb-content">' +
                  '<div class="ppc-qb-title">' +
                    (AR ? "طلب سريع" : "Quick Buy") +
                    '<span class="ppc-qb-badge">' + (AR ? "في 5 ثوانٍ" : "in 5 sec") + '</span>' +
                  '</div>' +
                  '<div class="ppc-qb-sub">' +
                    (AR ? "خطّ سريع — تخطّى السلة وادفع مباشرة" :
                          "Express lane — skip the cart, checkout instantly") +
                  '</div>' +
                '</div>' +
                '<div class="ppc-qb-arrow" aria-hidden="true">' + (AR ? "←" : "→") + '</div>';
            card.addEventListener("click", function () {
                // Forward the click to the original Fast Order button so all
                // of zorder.js's logic runs unchanged.
                var realBtn = zoWrap.querySelector(".zo-btn-open");
            ...
            // Mount the card right BEFORE the CTA row
            ctaRow.parentNode.insertBefore(card, ctaRow);
        }
    }
    ...
    function ppcWireReviewsTab() {
        var tabLink = document.querySelector('a[href="#ppc-tab-reviews"]');
        var host    = document.querySelector('[data-ppc-reviews-host="1"]');
        if (!tabLink || !host) return;
        var loaded = false;
        var moveReviews = function() {
            if (loaded) return;
            // Common reviews containers across Odoo + Theme Prime
            var src = document.querySelector(
                '#tp-product-rating-tab,#product-reviews,.o_portal_chatter,' +
                '.o_website_rating_card,.tp-reviews-section,.product_rating_box'
            );
            if (src) {
                host.innerHTML = '';
        ...
        tabLink.addEventListener('shown.bs.tab', moveReviews);
        // If Bootstrap events aren't available, also wire a plain click handler
        tabLink.addEventListener('click', function(){ setTimeout(moveReviews, 80); });
    }
