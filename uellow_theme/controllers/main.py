...
class ThemePrimeWebsiteSale(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', min_price=0.0, max_price=0.0, ppg=False, **post):
        ...
        if theme_id and theme_id.name.startswith('uellow_theme'):
            ...
            if _config_shop_layout.get('layout') == 'prime':
                response.template = 'uellow_theme.shop_layout'
    ...
    def cart(self, access_token=None, revive='', **post):
        ...
        if post.get('type') == 'tp_cart_sidebar_request':
            ...
            return request.render('uellow_theme.cart_sidebar', {'order': order}, headers={'Cache-Control': 'no-cache'})
        return res

    @http.route('/uellow_theme/get_search_sidebar', type='http', auth='public', website=True, sitemap=False)
    def search_sidebar(self, **post):
        return request.render('uellow_theme.search_sidebar')

    @http.route('/uellow_theme/get_categories_list', type='json', auth='public', website=True, sitemap=False)
    ...
    @http.route('/uellow_theme/get_quick_view_html', type='json', auth='public', website=True)
    def get_quick_view_html(self, options, **kwargs):
        ...
        result = request.website.get_uellow_theme_shop_config()
        values.update(result)
        values.update(extra)

        if options.get('variant_selector'):
            values['auto_add_product'] = product.product_variant_count == 1
            return IrUiView._render_template('uellow_theme.product_variant_selector_dialog', values=values)

        if options.get('right_panel'):
            return IrUiView._render_template('uellow_theme.tp_product_right_panel', values=values)
        return IrUiView._render_template('uellow_theme.tp_product_quick_view', values=values)
...
class DroggolComboConfiguratorController(WebsiteSaleComboConfiguratorController):
    ...
    def website_sale_combo_configurator_update_cart(self, *args, **kwargs):
        ...
        theme_id = request.website.sudo().theme_id
        if theme_id and theme_id.name.startswith('uellow_theme') and request.website._get_dr_theme_config('cart_flow') != 'default':
...
class DroggolWishlist(WebsiteSaleWishlist):
    @http.route('/uellow_theme/wishlist_general', auth="public", type='json', website=True)
...
class ThemePrimeMainClass(http.Controller):
    ...
    def _prepare_product_data(self, products, fields, pricelist, options=None):
        ...
        if not price_public_visibility:
            visibility_label = self._get_tp_view_template('uellow_theme.tp_b2b_price_label')
        ...
        for res_product, product in zip(result, products):
            ...
            if 'colors' in extra_data:
                res_product['colors'] = self._get_tp_view_template('uellow_theme.product_preview_swatches', {'product': product if res_model == 'product.template' else product.product_tmpl_id, '_limit': 4, 'parent_selector': '.card', 'img_selector': '.d-product-img'})
            # label and color
            if 'dr_label_id' in fields and product.dr_label_id:
                ...
                res_product['label_template'] = self._get_tp_view_template('uellow_theme.product_label', {'label': product.dr_label_id})
            if 'dr_stock_label' in extra_data and showStockLabel and product.dr_show_out_of_stock:
                res_product['dr_stock_label'] = self._get_tp_view_template('uellow_theme.product_label', {'product': product, 'stock': True})
    ...
    def _get_shop_related_data(self, options):
        ...
        if (options.get('shop_config_params')):
            shop_data['shop_config_params'] = request.website.get_uellow_theme_shop_config()
    ...
    def _get_rating_template(self, rating_avg, rating_count=False):
        return request.website._get_uellow_theme_rating_template(rating_avg, rating_count)
    ...
    @http.route('/uellow_theme/tp_create_collection', type='json', auth='user', website=True, sitemap=False)
    ...
    @http.route('/uellow_theme/tp_search_read', type='json', auth='user', website=True, sitemap=False)
        ...
        if model in ['product.template', 'product.product']:
            ...
            for tmpl in templateIDs:
                ...
                if view:
                    ...
                    if extras.get('registryToUse') == 'uellow_theme_snippet_registry' and imgSrc == 's_mega_menu_category_tabs_snippet':
                    ...
                    viewData.append({
                        ...
                        'imgSrc': '/uellow_theme/static/src/img/dynamic_snippet/%s.jpg' % imgSrc,
                        'icon': 'uellow_theme.icon_grid_1_2',
    ...
    @http.route('/uellow_theme/get_brands_category_data', type='json', auth='public', website=True)
    ...
    @http.route('/uellow_theme/get_listing_products', type='json', auth='public', website=True)
    ...
    @http.route('/uellow_theme/get_products_data', type='json', auth='public', website=True)
    ...
    @http.route('/uellow_theme/get_products_by_category', type='json', auth='public', website=True)
    ...
    @http.route('/uellow_theme/get_top_categories', type='json', auth='public', website=True)
    ...
    @http.route(['/uellow_theme/get_dialog_content'], type='json', website=True, auth="public")
    ...
    @http.route('/uellow_theme/get_categories_count', type='json', auth='public', website=True)
    def get_categories_count(self):
        return request.env['product.template']._get_product_category_count(domain=request.env['website'].sale_product_domain())

    @http.route('/uellow_theme/get_categories_info', type='json', auth='public', website=True)
    ...
    @http.route('/uellow_theme/get_brands', type='json', auth='public', website=True)
    ...
    @http.route('/uellow_theme/get_preview_body', type='http', auth="user", website=True, sitemap=False, multilang=False)
    def get_preview_body(self, **kw):
        return request.env['ir.ui.view']._render_template('uellow_theme.tp_preview_body', {'mobile': kw.get('mobile')})

    @http.route('/uellow_theme/get_megamenu_categories', type='json', auth='public', website=True)
...
class ThemePrimeBrand(http.Controller):
    ...
    def brands(self, search='', **args):
        ...
        return request.render('uellow_theme.all_brands', {'is_disable_grouping': is_disable_grouping, 'brands': brands, 'grouped_brands': grouped_brands, 'search': search, 'get_brand_count': get_brand_count})
...
class ThemePrimePWA(http.Controller):
    ...
    def get_pwa_service_worker(self, **kargs):
        ...
        data = file_open('uellow_theme/static/src/js/pwa/service_worker.js', 'rb').read().decode()
    ...
    def get_pwa_offline_page(self, **kargs):
        return request.render('uellow_theme.pwa_offline_page', {})
