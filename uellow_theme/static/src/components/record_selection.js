...
export class TpRecordSelector extends AbstractComponent {
    setup() {
        ...
        onWillStart(async () => {
            ...
            await loadJS("/uellow_theme/static/lib/Sortable/Sortable.js");
            ...
            if (this.environmentModel === 'product.public.category') {
                this.fetchedCategoryData = await this.componentService._fetchRecords({ options: { getCount: true, categoryIDs: this.recordsIDs }, fields: ['dr_category_label_id']}, { routePath: '/uellow_theme/get_categories_info'});
            }
            // magic happens here :)
    ...
    async _refreshRecords(recordsIDs) {
        ...
        this.records = await this.componentService._fetchRecords(params, { fieldsToMarkup: this.fieldsToMarkup });
        if (this.environmentModel === 'product.public.category') {
            this.fetchedCategoryData = await this.componentService._fetchRecords({ options: { getCount: true, categoryIDs: recordsIDs }, fields: ['dr_category_label_id'] }, { routePath: '/uellow_theme/get_categories_info' });
        }
    ...
    async _onClickCreateCollection() {
        await this.componentService._fetchRecords({ model: this.environmentModel, recordsIDs: this.recordsIDs, name: this.collectionInput.el.value }, { routePath: '/uellow_theme/tp_create_collection' });
...
TpRecordSelector.template = 'uellow_theme.TpRecordSelector';
