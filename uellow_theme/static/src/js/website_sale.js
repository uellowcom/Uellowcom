...
import { LazyContentDialog } from "@uellow_theme/js/dialog/lazy_content_dialog";
import { rpc } from "@web/core/network/rpc";
import { sprintf } from "@web/core/utils/strings";
import { ProductCarouselMixins } from "@uellow_theme/js/core/mixins";
import { Sidebar } from "@uellow_theme/js/sidebar/sidebar";
...
publicWidget.registry.WebsiteSale.include({
    jsLibs: (publicWidget.registry.WebsiteSale.prototype.jsLibs || []).concat(
        ['/uellow_theme/static/lib/drift-master-1.5.0/dist/Drift.js'],
    ),
    cssLibs: (publicWidget.registry.WebsiteSale.prototype.cssLibs || []).concat(
        ['/uellow_theme/static/lib/drift-master-1.5.0/dist/drift-basic.css'],
    ),
    ...
    _onChangeCombination: function (ev, $parent, combination) {
        ...
        if (combination.product_price_offer && typeof combination.product_price_offer === "object") {
            $(renderToElement("uellow_theme.ProductPriceOffer", combination.product_price_offer)).appendTo($productPriceOfferContainer);
        ...
        if (combination.bulk_price && combination.bulk_price.length) {
            ...
            if (doRenderPrices) {
                $(renderToElement("uellow_theme.BulkPrice", { prices: combination.bulk_price, inputQty: parseInt($parent.find('input.quantity').val()), isQuickViewDialog: this.isQuickViewDialog, markup, parseInt, })).appendTo($bulkPriceContainer);
            }
        }
        // Extra Fields
    ...
    _onClickOpenFilterSidebar: function (ev) {
        ...
        const sidebarHtml = renderToElement("uellow_theme.ShopFilterSidebar", { content: markup($(".tp-filters-container")[0].innerHTML) });
    ...
    _replaceContent: function (url) {
        ...
        this.$shopLoader = $(renderToElement('uellow_theme.Loader'));
...
publicWidget.registry.TpRangeFilter = publicWidget.Widget.extend({
    ...
    jsLibs: ['/uellow_theme/static/lib/ion.rangeSlider-2.3.1/js/ion.rangeSlider.js'],
    cssLibs: ['/uellow_theme/static/lib/ion.rangeSlider-2.3.1/css/ion.rangeSlider.css'],
    events: {
...
publicWidget.registry.TpLazyContent = publicWidget.Widget.extend({
    ...
    _onClick: async function (ev) {
        ...
        if (!this.content) {
            const result = await rpc('/uellow_theme/get_dialog_content', {
                res_id: resId,
                res_model: resModel,
