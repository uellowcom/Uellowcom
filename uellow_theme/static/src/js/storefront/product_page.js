...
(function () {
    ...
    function bindDescBtns() {
        ...
        // PHASE 13: AR/EN language switch in the description card head.
        // Each pill carries data-lb="ar"|"en"; clicking it shows the
        // matching ppc-desc-block and hides the other.
        document.querySelectorAll(".ppc-desc-langpill").forEach(function(pill){
            if (pill._b) return; pill._b = true;
            pill.addEventListener("click", function(){
                var card = pill.closest(".ppc-desc-card"); if (!card) return;
                var lb = pill.getAttribute("data-lb") || "en";
                card.querySelectorAll(".ppc-desc-langpill").forEach(function(p){
                    p.setAttribute("aria-pressed", p.getAttribute("data-lb") === lb ? "true" : "false");
                });
                card.querySelectorAll(".ppc-desc-block").forEach(function(b){
                    if (b.getAttribute("data-lb") === lb) {
                        b.classList.remove("ppc-hidden");
                    } else {
                        b.classList.add("ppc-hidden");
                    }
                });
            });
        });
    ...
    /* ══════════════════════════════════════════════════════════════
       PHASE 14 — Relocate WhatsApp + Fast Order out of the main CTA row
       ──────────────────────────────────────────────────────────────
       The CTA row in the theme is `[qty] [Add to Cart] [WhatsApp] [Fast Order]`.
       Mixing four buttons makes the row noisy and pushes Add to Cart down on
       mobile. We move:
         • WhatsApp  → a fixed floating bubble at the bottom-left of the page
         • Fast Order → a "Quick Buy" callout card placed ABOVE the CTA row
       Both relocated nodes get a `.ppc-mounted` class so CSS can show them.
       ══════════════════════════════════════════════════════════════ */
    function ppcRelocateCTAExtras() {
        // Goal: a single tidy CTA row on desktop AND mobile.
        //   [ qty ]  [ Add to Cart ]  [ Fast Buy ]  [ WhatsApp ]
        // - Move `.zo-wrap` (Fast Buy from the zorder module) INTO the CTA
        //   row, right after #add_to_cart so it sits beside it.
        // - Strip the module's inline `style="..."` on the button + wrapper
        //   (those gradients/colors fight our design) and add stable
        //   classes so the CSS in product_page.css can theme all three
        //   buttons consistently with only color variations.
        // - WhatsApp is wrapped via XML; CSS shrinks its text away under
        //   the mobile breakpoint so only the icon remains.
        var atcWrap = document.getElementById("add_to_cart_wrap");
        var atc     = document.getElementById("add_to_cart");
        var zoWrap  = document.querySelector(".zo-wrap");
        var wa      = document.getElementById("ppcWaBtn");
        if (!(atcWrap && atc)) return;

        if (zoWrap && !zoWrap.classList.contains("ppc-fastbuy-mounted")) {
            // Reposition the FastBuy wrapper inside the CTA row, just
            // after Add to Cart. We KEEP zo-wrap in the tree (zorder.js
            // binds to .zo-btn-open inside it) but move it.
            if (atc.nextSibling !== zoWrap) {
                atc.parentNode.insertBefore(zoWrap, atc.nextSibling);
            }
            // Drop inline positioning/margin so flex layout owns spacing.
            zoWrap.style.cssText = "";
            zoWrap.classList.add("ppc-fastbuy-wrap", "ppc-fastbuy-mounted");

            // Strip the inline-styled "in 5 sec" badge — too noisy now.
            var oldBadge = zoWrap.querySelector("span[style*='in 5 Sec'], span[style*='red']");
            // First child span is the "in 5 sec" pill (set via inline style)
            var firstSpan = zoWrap.querySelector(":scope > span");
            if (firstSpan) firstSpan.remove();

            var btn = zoWrap.querySelector(".zo-btn-open");
            if (btn) {
                btn.style.cssText = "";
                btn.classList.add("ppc-cta-btn", "ppc-cta-fastbuy");
                btn.setAttribute("aria-label", AR ? "اشترِ الآن" : "Buy Now");
                // Replace inner content with a clean icon + label
                btn.innerHTML =
                    '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">' +
                        '<path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z"/>' +
                    '</svg>' +
                    '<span class="ppc-cta-label">' + (AR ? "اشترِ الآن" : "Buy Now") + '</span>';
            }
        }

        // Mark Add to Cart + WA with the same unified class so they pick
        // up the shared button shape from CSS.
        if (atc && !atc.classList.contains("ppc-cta-btn")) {
            atc.classList.add("ppc-cta-btn", "ppc-cta-primary");
        }
        if (wa && !wa.classList.contains("ppc-cta-btn")) {
            wa.classList.add("ppc-cta-btn", "ppc-cta-wa");
        }
    }
    ...
    function init() {
        ...
        // PHASE 14 — relocate WhatsApp + Fast Order out of the CTA row.
        // Done early so users never see them flash inside the row.
        try { ppcRelocateCTAExtras(); } catch(e) {}
        // 150ms: UI elements
        setTimeout(function(){
        ...
        setTimeout(function(){
            ...
            try { ppcWireReviewsTab();  } catch(e) {}
            try { enhanceBulkOrder(); } catch(e) {}
            // Second pass: in case zorder.js / WA template inserted late
            try { ppcRelocateCTAExtras(); } catch(e) {}
        }, 400);
        // 800ms: async load
        setTimeout(function(){
    ...
    /* ── 0. Product ID + view count + stock pill next to ratings ── */
    function addProductId(){
        ...
        // View count pill — real cumulative views from website.track
        if(!wrap.querySelector(".ppc-view-count")){
            var views = parseInt(window.ppcViews || 0, 10) || 0;
            var vBadge = document.createElement("span");
            vBadge.className = "ppc-view-count";
            vBadge.title = AR ? "عدد المشاهدات" : "Page views";
            vBadge.innerHTML =
                '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" ' +
                'stroke="currentColor" stroke-width="2" stroke-linecap="round" ' +
                'stroke-linejoin="round" style="vertical-align:-2px;margin-inline-end:3px">' +
                '<path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/>' +
                '<circle cx="12" cy="12" r="3"/></svg>' +
                views + ' ' + (AR ? 'مشاهدة' : (views === 1 ? 'view' : 'views'));
            wrap.appendChild(vBadge);
        }

        // Stock pill — read once after stock module renders
        setTimeout(function(){
            ...
            // Hide theme_prime viewers — we render our own
            document.querySelectorAll(".tp-views-indicator").forEach(function(el){
                try{
    ...
    function buildBanner() {
        // Server-rendered installments card (#ppcInstall) is the source
        // of truth — translatable through ir.ui.view.arch_db, supports
        // all site languages. Skip the JS-built fallback when present.
        if (document.getElementById("ppcInstall")) return;
    ...
    function loadTopSelling(){
        ...
        fetch("/ppc/top_selling",{method:"POST",headers:{"Content-Type":"application/json"},
            body:JSON.stringify({jsonrpc:"2.0",method:"call",id:Date.now(),params:{product_id:window.ppcProd||0,categ_id:window.ppcCatg||0}})
        }).then(function(r){return r.json();}).then(function(j){
    ...
    /* ── PHASE 13: lazy-load reviews into our new Reviews tab ─────
       Finds the existing Odoo reviews block (rendered elsewhere on the
       page by the theme/website_sale.product_comment template) and moves
       it inside the .ppc-tabs-card Reviews pane when the user opens it. */
    function ppcWireReviewsTab() {
        // The .prw widget now renders directly inside #ppc-tab-reviews via
        // QWeb (see storefront_website_sale.xml). Nothing to move. We just
        // hide any duplicate native reviews block that Odoo renders elsewhere
        // on the page so the .prw appears only once.
        var dupes = document.querySelectorAll(
            '.o_shop_discussion_rating, #o_product_page_reviews,' +
            '#tp-product-rating-tab > .prw'
        );
        dupes.forEach(function(el){
            // Skip the one inside our tab
            if (el.closest('#ppc-tab-reviews')) return;
            el.style.display = 'none';
        });
    }

    /* ── 9. Move tabs into our layout (PHASE 13: now also handles ppc-tabs-card) ─
       We render our own .ppc-tabs-card in the template that replaces the
       theme's .tp-product-details-tab. This function moves whichever one
       happens to be on the page into #ppcTabsCol, so the layout stays the
       same regardless of which template chain is active. */
    function moveTabsToLayout(){
        setTimeout(function(){
            try {
                var col=document.getElementById("ppcTabsCol"); if(!col)return;
                var tabs = document.querySelector(".ppc-tabs-card") ||
                           document.querySelector(".tp-hook-product-tabs .tp-product-details-tab") ||
                           document.querySelector(".tp-product-details-tab");
                if(tabs && tabs.parentNode !== col) col.appendChild(tabs);
