...
export const Sidebar = publicWidget.Widget.extend({
    ...
    _renderLoader: function () {
        this.$loader = $(renderToElement("uellow_theme.Loader", {loadingStr: this.options.loadingStr, height: "100vh"}));
...
publicWidget.registry.TpSearchSidebarBtn = publicWidget.Widget.extend({
    ...
    _onClick: function (ev) {
        ...
        return new SearchSidebar(this, {
            ...
            fetchUrl: "/uellow_theme/get_search_sidebar",
            position: ev.currentTarget.dataset.position || "end",
        }).show();
    },
});
...
export const CategorySidebar = Sidebar.extend({
    ...
    _appendContent: async function () {
        this._renderLoader();
        const data = await rpc("/uellow_theme/get_categories_list");
    ...
    _renderCategories: function (categories, parentId=false) {
        ...
        const $categories = $(renderToElement("uellow_theme.CategorySidebar", { categories, parentCategory, categoryCount: this.categoryCount, options: this.options, config: odoo.dr_theme_config.json_sidebar_config }));
        this.$el.empty();
...
publicWidget.registry.TpSimilarProductSidebarBtn = publicWidget.Widget.extend({
    ...
    _onClick: function (ev) {
        ...
        return new Sidebar(this, {
            ...
            fetchUrl: "/uellow_theme/get_similar_products_sidebar",
