...
import { TpSnippetConfigDialog } from "@uellow_theme/components/dialog";
...
import { _t } from "@web/core/l10n/translation";
    ...
    options.registry.s_tp_documents_snippet = options.Class.extend({
        ...
        */
        ...
        _appendDocument: function (documentInfo) {
            ...
            this.$target.find('.s_tp_documents_snippet_row').append($(renderToElement('uellow_theme.s_tp_documents_snippet_template', { documentInfo: documentInfo })));
        },
    });
    ...
    options.registry.TpImageHotspot = options.Class.extend({
        ...
        addHotspot(previewMode, widgetValue, params) {
            ...
            let values = { hotspotType: 'static', titleText: "Your title", subtitleText: "Theme prime is best theme", buttonLink: '/', buttonText: 'SHOP NOW', imageSrc: '/uellow_theme/static/src/img/s_config_data.png' };
    ...
    options.registry.TpImageHotspotConfig = options.Class.extend({
        ...
        // xmlDependencies: ['/uellow_theme/static/src/xml/frontend/image_hotspot.xml', '/uellow_theme/static/src/xml/editor/widgets/products_widget.xml'],
        /**
         * @override
         */
        start: function () {
        ...
        _openPopover: function () {
            if (this.PreviewEnabled) {
                this.$target.popover({
                    ...
                    html: true,
                    trigger: 'manual',
                    content: renderToElement('uellow_theme.tp_img_static_template', { widget: this, data: this._getHotspotConfig() }),
    ...
    options.registry.tp_dynamic_snippet = options.Class.extend({
        ...
        */
        async willStart() {
            const _super = this._super.bind(this);
            this.registryToUse = this.$target[0].parentElement.classList.contains('o_mega_menu') ? 'uellow_theme_mega_menus' : 'uellow_theme_snippet_registry';
            if (this.registryToUse === 'uellow_theme_snippet_registry') {
                this.switchableRelatedViews = await this._rpc.call("/uellow_theme/tp_search_read", { model: 'ir.ui.view', extras: { templateIDs: ['uellow_theme.s_tp_categories_menu'] } });
            }
            return _super(...arguments);
        },
        /**
        ...
        */
        ...
        _setConfiguratorParams: function (data) {
            ...
            if (data.$target && !isSameSnippet && data.templateID === 's_mega_menu_category_tabs_snippet' && this.registryToUse === 'uellow_theme_snippet_registry') {
