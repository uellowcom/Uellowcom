...
class MobileNotificationController(http.Controller):
    ...
    def get_notifications(self, **kwargs):
        ...
        try:
            ...
            # Get notifications
            notifications = (
                request.env["mobile.notification"]
                .sudo()
            ...
            total_count = (
                request.env["mobile.notification"]
                .sudo()
                .search_count([("partner_id", "=", current_user.id)])
            )
            unread_count = (
                request.env["mobile.notification"]
                .sudo()
    ...
    @http.route(
        "/mobile/v1/notifications/<int:notification_id>/read",
        auth="public",
        methods=["POST"],
        type="json",
        csrf=False,
    ...
    @http.route(
        "/mobile/v1/notifications/read-all",
        auth="public",
        methods=["POST"],
        type="json",
        csrf=False,
    ...
    def mark_all_notifications_read(self):
        ...
        try:
            ...
            unread_notifications = (
                request.env["mobile.notification"]
                .sudo()
                .search([("partner_id", "=", current_user.id), ("is_read", "=", False)])
            )
    ...
    @http.route(
        "/mobile/v1/notifications/settings",
        auth="public",
        methods=["GET"],
        type="http",
        csrf=False,
    ...
    def get_notification_settings(self):
        ...
        try:
            ...
            return self._create_response(
                error="Failed to fetch notification settings", status=500
            )
    ...
    def update_notification_settings(self):
        ...
        try:
            ...
            return self._create_response(
                error="Failed to update notification settings", status=500
            )

    @http.route(
        "/mobile/v1/notifications/register-device",
        auth="public",
        methods=["POST"],
        type="json",
        csrf=False,
        ...
        try:
            ...
            if device:
                ...
                device_vals = {
                    "partner_id": current_user.id,
                    "device_id": device_id
                    or f"{device_type}_{int(datetime.now().timestamp())}",
                    "device_type": device_type,
                    "push_token": device_token,
                    "is_active": True,
                    "last_used": fields.Datetime.now(),
            ...
            return self._create_response(
                error="Failed to register device token", status=500
            )

    @http.route(
        "/mobile/v1/notifications/send-test",
        auth="public",
        methods=["POST"],
        type="json",
        csrf=False,
    ...
    def send_test_notification(self):
        ...
        try:
            ...
            notification = (
                request.env["mobile.notification"].sudo().create(notification_vals)
            )
            ...
            return self._create_response(
                error="Failed to send test notification", status=500
            )
