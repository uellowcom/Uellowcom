...
class MobileAuthController(http.Controller):
    ...
    @http.route('/mobile/v1/auth/register', auth='public', methods=['POST'], type='json', csrf=False)
    def register(self):
    ...
    @http.route('/mobile/v1/auth/login', auth='public', methods=['POST'], type='json', csrf=False)
    def login(self):
    ...
    @http.route('/mobile/v1/auth/firebase-sms', auth='public', methods=['POST'], type='json', csrf=False)
        ...
        try:
            ...
            if data.get('verification_code') and data.get('verification_id'):
                try:
                    ...
                    if not result:
                        return self._create_response(error="Invalid verification code", status=400)
                    ...
                    return self._create_response(error="SMS verification failed", status=400)
            
            else:
                # Send SMS verification code
                try:
                    verification_id = firebase_service.send_sms_verification(data['phone_number'])
    ...
    @http.route('/mobile/v1/auth/social/<string:provider>', auth='public', methods=['POST'], type='json', csrf=False)
    def social_login(self, provider):
        ...
        try:
            ...
            try:
                ...
                user_info = firebase_service.verify_social_token(provider, data['id_token'])
                
                if not user_info:
                    return self._create_response(error="Invalid social login token", status=401)
                ...
                return self._create_response(error="Social login verification failed", status=401)

        except Exception as e:
            _logger.error(f"Social login error: {str(e)}")
            return self._create_response(error="Social login failed", status=500)

    @http.route('/mobile/v1/auth/refresh', auth='public', methods=['POST'], type='json', csrf=False)
    ...
    @http.route('/mobile/v1/auth/logout', auth='public', methods=['POST'], type='json', csrf=False)
    def logout(self):
        """Logout current user"""
        try:
            ...
            if not auth_header or not auth_header.startswith('Bearer '):
                return self._create_response(error="Missing or invalid authorization header", status=401)
    ...
    @http.route('/mobile/health', auth='public', methods=['GET'], type='http', csrf=False)
