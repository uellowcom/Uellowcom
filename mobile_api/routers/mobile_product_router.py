...
async def get_products(
    ...
    try:
        ...
        if sort_by == "price":
            ...
            order_by = (
                "rating_avg"
                if hasattr(env["product.product"], "rating_avg")
                else "name"
            )
        ...
        products = env["product.product"].search(
            domain, limit=limit, offset=offset, order=order_by
        )
        ...
        for product in products:
            ...
            in_wishlist = False
            if current_user:
                in_wishlist = env["mobile.wishlist"].is_in_wishlist(
            ...
            result.append(
                ProductResponse(
                    ...
                    brand=(
                        ...
                        if hasattr(product, "product_brand_id")
                        and product.product_brand_id
                        else None
                    ),
...
async def get_product_detail(
    ...
    try:
        ...
        return ProductDetail(
            ...
            rating=product.rating_avg if hasattr(product, "rating_avg") else 0,
            review_count=(
                product.rating_count if hasattr(product, "rating_count") else 0
            ),
            in_wishlist=in_wishlist,
            variants=variants,
...
async def get_product_categories(env: Annotated[Environment, Depends(odoo_env)]):
            ...
            result.append(
                {
                    ...
                    "parent_name": (
                        category.parent_id.name if category.parent_id else None
                    ),
...
async def get_product_by_barcode(
    ...
    try:
        ...
        product_data = product.get_mobile_product_data(
            product.id, current_user.id if current_user else None
        )
        return product_data
...
async def get_recent_viewed_products(
    ...
    try:
        recent_products = env["mobile.product.view"].get_recent_viewed(
