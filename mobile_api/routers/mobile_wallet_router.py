...
async def get_transaction_history(
    ...
    transaction_type: str = Query(None, pattern="^(credit|debit)$"),
):
    """Get wallet transaction history"""
    try:
        transactions = env["mobile.wallet.transaction"].get_transaction_history(
            partner_id=current_user.id, limit=limit, transaction_type=transaction_type
        )
...
async def get_wallet_stats(
    ...
    try:
        ...
        total_credits = sum(
            txn.amount for txn in transactions if txn.transaction_type == "credit"
        )
        total_debits = sum(
            abs(txn.amount) for txn in transactions if txn.transaction_type == "debit"
        )
