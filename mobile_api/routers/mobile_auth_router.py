...
async def register(
    ...
    try:
        ...
        existing_user = env['res.partner'].search([('email', '=', user_data.email)], limit=1)
        if existing_user:
            raise HTTPException(
...
async def login(
    ...
    try:
        ...
        if credentials.device_id:
            env['mobile.device'].register_device(partner.id, {
...
async def firebase_sms_auth(
    ...
    try:
        ...
        if data.verification_code:
            ...
            # Generate JWT tokens
            jwt_service = JWTService()
            tokens = jwt_service.create_tokens(partner.id, partner.email or data.phone_number)
            ...
            verification_id = await firebase_service.send_sms_verification(data.phone_number)
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="SMS authentication failed"
        )
