...
class MobileNotification(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner", string="Recipient", required=True, ondelete="cascade"
    )
    ...
    def _send_push_notification(self):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Would send push notification to {partner.id}: "
            ...
            self.write(
                {
                    ...
                    "push_error": (
                        None if success else "Failed to send push notification"
                    ),
                }
            )
            ...
            self.write({"push_sent": False, "push_error": str(e)})
            return False
    ...
    @api.model
    def notify_wallet_transaction(
        self, partner_id, transaction_id, amount, transaction_type
    ):
    ...
    def send_bulk_notification(
        self, partner_ids, title, message, notification_type="info", data=None
    ):
    ...
    def cleanup_old_notifications(self, days=30):
        ...
        old_notifications = self.search(
            [("create_date", "<", cutoff_date), ("is_read", "=", True)]
        )
...
class ResPartnerNotification(models.Model):
    ...
    notification_ids = fields.One2many(
        "mobile.notification", "partner_id", string="Notifications"
    )
    unread_notification_count = fields.Integer(
        "Unread Notifications", compute="_compute_unread_notifications"
    )

    def _compute_unread_notifications(self):
        """Compute unread notification count"""
        for partner in self:
            partner.unread_notification_count = self.env[
                "mobile.notification"
            ].search_count([("partner_id", "=", partner.id), ("is_read", "=", False)])

    def send_notification(
        self, title, message, notification_type="info", data=None, send_push=True
    ):
