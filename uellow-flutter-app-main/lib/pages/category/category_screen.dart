...
class CategoryScreen extends StatefulWidget {
  ...
  bool isFromSplash;
  CategoryScreen({Key? key, this.isFromHome, this.isFromSplash = false})
      : super(key: key);
...
class _CategoryScreenState extends State<CategoryScreen>
  ...
  loadAllCategories() async {
    ...
    await Provider.of<CategoryProvider>(context, listen: false)
        .fetchAllCategories(context, widget.isFromSplash)
  ...
  loadPopularCategories() async {
    await Provider.of<CategoryProvider>(context, listen: false)
        .fetchPopularCategories(context);
  }

  loadSubCategories() async {
    await Provider.of<CategoryProvider>(context, listen: false)
        .fetchSubCategories(chosenindex, page, context);
  }

  loadProducts() async {
    await Provider.of<CategoryProvider>(context, listen: false)
        .fetchProductsCategory(chosenindex.toString(), context);
  }
  ...
  Widget build(BuildContext context) {
                  ...
                  Consumer<HomeProvider>(
                    builder: (context, value, child) {
                      return BannerContainer(
                          dataSliderLength: value.bannerCategories.length,
                          contentHeight: MediaQuery.of(context).size.height,
                          isFromCategory: true,
                          dataSlider: value.bannerCategories,
                          ...
                          Navigator.push(
                              ...
                              MaterialPageRoute(
                                  builder: (context) => BrandProducts(
                                        ...
                                        countSub: value.subCategories[i].count,
                                      )));
                        },
                        child: Column(
                          children: [
    ...
    Widget subViewProducts = Consumer<CategoryProvider>(
      builder: (context, value, child) {
        if (value.loadingSub) {
          return subShimmer();
        }
            ...
            Container(
              ...
              child: GridView.builder(
                ...
                gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                    ...
                    crossAxisCount: 2,
                    childAspectRatio:
                        locale == Locale('ar') ? 68.h / 153.h : 68.h / 143.h),
                itemBuilder: (context, i) {
  ...
  Widget tabBar(
      String title, String image, int? indexTab, int? countSub, int index) {
    final categories = Provider.of<CategoryProvider>(context, listen: false);
    return InkWell(
      onTap: () {
        setState(() {
        ...
        printLog("$chosenCountSub", name: "countSub");
        if (chosenindex == 9911) {
