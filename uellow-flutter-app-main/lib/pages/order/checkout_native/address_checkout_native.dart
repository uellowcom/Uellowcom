...
class AddressCheckoutNative extends StatefulWidget {
  final dynamic data;
  final String? title;
  bool? isGuest = false;
  ...
  @override
  _AddressCheckoutNative createState() => _AddressCheckoutNative();
}

class _AddressCheckoutNative extends State<AddressCheckoutNative> {
  ...
  void initState() {
    ...
    if (!Session.data.getBool('isLogin')!) {
      if (widget.billingEmpty) {
        userProvider!.fetchCountries(context).then((value) {
          if (value) {
  ...
  loadDetail() async {
    await context.read<UserProvider>().fetchAddress(context);
    await context.read<UserProvider>().fetchCountries(context);
    checkBilling();
  }
  ...
  checkBilling() {
    ...
    setState(() {
      if (_billing.code != null) {
        countryCode = "+${_billing.code}";
      }
    });
                      ...
                      children: [
                        ...
                        ContainerCard(
                          ...
                          children: [
                            ...
                            Visibility(
                                visible:
                                    widget.title!.toLowerCase() == 'billing',
                                child: Column(
                                  children: [
                                    Row(
                                      children: [
                          ...
                          children: [
                            ...
                            value.subdistrict.isEmpty || value.cities.isEmpty
                                ? FormRevo(
                                    ...
                                    label:
                                        '${AppLocalizations.of(context)!.translate('apartement_suite')}',
                                    hint:
                                        '${AppLocalizations.of(context)!.translate('apartement_suite')}',
                                  )
                                : Container(
                                    child: value.loadingSub
                                        ? customLoading()
                                        : _buildDropdown(
                                            '${AppLocalizations.of(context)!.translate('subdistrict')}*',
                                            value,
                                            'subdistrict'),
                                  ),
                            FormRevo(
