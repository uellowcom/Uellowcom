...
def migrate(cr, version):
    ...
    views_id_to_delete = ["uellow_theme_common"]
    domain = [('arch_fs', 'ilike', f'{view}/%') for view in views_id_to_delete]
    ors = ['|'] * (len(domain) - 1)
    domain = ors + domain
    delete_views(domain, env)
    paths = ['/uellow_theme_common/static/src/js/backend/res_config_settings.js', '/uellow_theme_common/static/src/scss/variants.scss', '/uellow_theme_common/static/src/js/backend/list_view_brand.js']
