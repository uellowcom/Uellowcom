...
class ProductPublicCategory(models.Model):
    ...
    # ─── Uellow mega-menu featured card override ──────────────────────
    # When empty, the mega menu falls back to `image_1024`. When the
    # marketing team wants a bespoke promo (custom artwork + landing
    # URL + copy), they fill these — no code changes required.
    uc_mega_image = fields.Binary(
        'Mega Menu Image',
        help='Custom image shown on the featured promo card of the '
             'mega menu for this category. Falls back to image_1024.')
    uc_mega_link = fields.Char(
        'Mega Menu Link',
        help='Optional URL the promo card links to. Empty = link to '
             'the category page itself.')
    uc_mega_eyebrow = fields.Char(
        'Mega Menu Eyebrow', translate=True,
        help='Small tag above the title on the promo card '
             '(default: "Featured"). Translatable.')
    uc_mega_tagline = fields.Char(
        'Mega Menu Tagline', translate=True,
        help='Short description on the promo card. Translatable.')
