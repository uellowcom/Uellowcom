...
async def get_home_data(
    ...
    try:
        ...
        # Get banners
        if hasattr(env, 'ref') and env.ref('website.carousel_snippet_options', raise_if_not_found=False):
            # If website module is installed, get banners from website carousel
            carousel_slides = env['website.carousel.slide'].sudo().search(
                [('is_published', '=', True)], 
                order='sequence'
            )
            ...
            for slide in carousel_slides:
                ...
                if slide.url and '/product/' in slide.url:
                    ...
                    try:
                        category_id = int(slide.url.split('/category/')[1].split('/')[0])
                        category = env['product.public.category'].sudo().browse(category_id)
                        if category.exists():
        ...
        featured_products = env['product.template'].sudo().search(
        ...
        best_selling = env['product.template'].sudo().search(
        ...
        discounted_products = env['product.template'].sudo().search(
        ...
        if discounted_products:
            ...
            for product in discounted_products:
                if product.list_price > product.price and product.list_price > 0:
                    discount = round((1 - (product.price / product.list_price)) * 100, 2)
...
async def get_search_suggestions(
    ...
    try:
        ...
        products = env['product.template'].sudo().search(
            [('website_published', '=', True), ('sales_count', '>', 0)], 
            limit=5, order='sales_count desc'
        )
