...
def format_product(env, product, partner=None, include_variants=False):
    ...
    variants = []
    if include_variants and product.product_variant_ids and len(product.product_variant_ids) > 1:
        for variant in product.product_variant_ids:
            ...
            variants.append({
                ...
                'in_stock': variant.sudo().qty_available > 0 if variant.type != 'service' else True,
                'qty_available': variant.sudo().qty_available if variant.type != 'service' else 0
            })
    
    # Check if product is in wishlist
    is_in_wishlist = False
    if partner:
        wishlist = env['product.wishlist'].sudo().search([
    ...
    return {
        ...
        'in_stock': in_stock,
        'qty_available': product.sudo().qty_available if product.type != 'service' else 0,
        'images': images,
...
async def get_categories(
    ...
    current_user: Annotated[Optional[Dict[str, Any]], Depends(get_optional_user)] = None
):
...
async def toggle_wishlist(
    ...
    try:
        ...
        product_id = product.product_variant_id.id if product.product_variant_id else product.product_variant_ids[0].id
...
async def get_wishlist(
    ...
    try:
        ...
        # Get wishlist items
        wishlist_model = env['product.wishlist'].sudo()
        wishlist_items = wishlist_model.search([
            ('partner_id', '=', partner_id)
        ])
